...
const AddressesView =({addresses,setAddresses,deleteAddress})=>{
...
(<div onClick={()=>{handleClick(address._id)}}className="address-item not-selected">{address.address}
<Button  onClick={()=>{deleteAddress(localStorage.token,address._id)}} startIcon={<DeleteIcon />}>
Delete
</Button></div>):
(<div onClick={()=>{handleClick(address._id)}}className="address-item selected">{address.address}
<Button  onClick={()=>{deleteAddress(localStorage.token,address._id)}} startIcon={<DeleteIcon />}>
Delete
</Button></div>)
  ...
  return (
    <Box display="flex" flexDirection="column">
      ...
      <Stack direction="row" my="1rem">
        <Button
          variant="contained"
          onClick={async()=>{await addAddress(token,addressValue);
...
const Checkout = () => {
  ...
  const deleteAddress = async (token, addressId) => {
    try {
      ...
      //   setAddresses({
      //     ...addresses, // copy the current addresses object
      //     selected:"" , // set the new selected value
      //   })
          ...
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              {/* TODO: CRIO_TASK_MODULE_CHECKOUT - Display list of addresses and corresponding "Delete" buttons, if present, of which 1 can be selected */}
               {addresses.all.length?(<div><AddressesView addresses={addresses} setAddresses={setAddresses} deleteAddress={deleteAddress}/></div>):  
               (<Typography my="1rem">
            ...
            <Button
              ...
              variant="contained"
              onClick={()=>{if(validateRequest(items,addresses))
